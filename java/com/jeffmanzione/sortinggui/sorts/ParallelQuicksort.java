...
@SortInfo(name = "Parallel Quicksort (End Pivot)", designer = "Hoare (1961)",
  ...
  private class SimulatedThread {
    ...
    private void nextMove(Iterator<SimulatedThread> iter) {
      ...
      if (j == p) {
        ...
        swap(p, i + 1);
        if (range.end - (i + 2) > 0) {
          nextRanges.add(new SimulatedThread(new Range(i + 2, range.end)));
        }
        if (i - range.start > 0) {
          nextRanges.add(new SimulatedThread(new Range(range.start, i)));
        }
        sorting = false;
