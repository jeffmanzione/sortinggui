...
public abstract class Heapsort extends Sort {
  ...
  protected void initialize() {
    ...
    heapify = false;
    index = curr_index =
        (int)Math.pow(2, (int)(Math.log10(heapSize) / Math.log10(2)) - 1) - 1 +
        (heapSize -
         (int)Math.pow(2, (int)(Math.log10(heapSize) / Math.log10(2))) + 1 +
         1) /
            2;
  }

  protected int parent(int index) { return (int)(index - 1) / 2; }

  protected int leftChild(int index) { return index * 2 + 1; }

  protected int rightChild(int index) { return index * 2 + 2; }
  ...
  protected boolean doneCheck() { return heapSize == 1; }
}
